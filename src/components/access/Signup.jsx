...
const Signup = () => {
 ...
 useEffect(() => {
  ...
  if (selectedCountry?.id) {
   const fetchCities = async () => {
    try {
     const serverRes = await getCitiesByCountry(
      selectedCountry.id,
      controller.signal,
     );
 ...
 const handlePhoneCodeChange = e => {
  ...
  if (inputCode && countryData) {
   const foundCountry = countryData.find(elem => elem.phonecode === inputCode);
      ...
      <form onSubmit={handleSubmit} className={classes.form}>
       <div className={classes.ep}>
        ...
        <Button
         ...
         type='submit'
         disabled={isLoading}>
         {isLoading ? <LoadingSpinner size='20px' /> : t('signup.sign_up')}
        </Button>
       </div>
      </form>
