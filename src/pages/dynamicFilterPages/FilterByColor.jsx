...
const FilterCategory = ({ windowSize }) => {
 ...
 const handleCheckboxChange = (e, slideId) => {
  setSelectedIds(prevIds =>
   e.target.checked ? [...prevIds, slideId] : prevIds.filter(id => id !== slideId),
  );
 };
 ...
 // Initial data load
 useEffect(() => {
  document.title = t('filter_by_category');
   ...
   <div className={classes.main}>
    ...
    <Body>
     <p
      ...
      style={{ fontSize: '1rem', margin: '2rem auto', width: 'fit-content' }}>
      Error: {error}
     </p>
    </Body>
    <Footer />
   ...
   <Body>
    <Card className={`${classes.size_wrapper} ${classes.colors_wrapper}`}>
     <Breadcrumbs
      linkDataProp={[
       { pathname: t('home'), url: '/' },
       { pathname: t('filter_by_category'), url: '/filterbycategory' },
      ]}
     />
     ...
     <div className={classes.thumbnail_container}>
      {sortedGroupColors.map(slide => (
       <div
        ...
        onKeyDown={e => e.key === 'Enter' && handleThumbClick(slide.id)}>
    ...
    <Card
     ...
     {lastPage > 1 && memoizedProducts.length > 0 && itemIds.length === 0 && (
      <div
