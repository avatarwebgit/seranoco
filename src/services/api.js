...
export const getAllProductFromCategory = async (
  category_alias,
  color_ids,
  shape_id,
  size_ids,
  ...
  const response = await fetch(
    `${baseUrl}/product/category/filter?page=${page}&per_page=${per_page}`,
    {
      ...
      body: JSON.stringify({ category_alias, color_ids, shape_id, size_ids }),
    }
  );
  const result = await response.json();
  return { response, result };
...
export const contactUsSend = async (name, email, message, mobile) => {
